...
func (t *Tunnel) startOpenVPN(dir string) error {
	ns := t.nsName()
	// A reconnect can briefly leave redirect-gateway routes from the old VPN.
	// Pin the new server to the veth gateway so its control connection can
	// never be sent into the stale tun0 route.
	if net.ParseIP(t.Node.IP) == nil {
		return fmt.Errorf("节点地址无效: %q", t.Node.IP)
	}
	peer := fmt.Sprintf("fop%d", t.Slot)
	if err := runInNetns(ns, "ip", "route", "replace", t.Node.IP+"/32",
		"via", t.subnet()+".1", "dev", peer); err != nil {
		return fmt.Errorf("固定 VPN 服务器路由失败: %w", err)
	}
...
func (t *Tunnel) serve() error {
	...
	var err error
	for i := 0; i < 6; i++ {
		ln, err = net.Listen("tcp", net.JoinHostPort(t.BindAddress, fmt.Sprint(t.Port)))
		if err == nil {
			break
		}
		time.Sleep(time.Second)
	...
	if err != nil {
		// 确实被别的进程长期占用了，才换端口
		port, perr := freeRandomPort(map[int]bool{t.Port: true}, t.BindAddress)
		if perr != nil {
			return fmt.Errorf("监听 %d 失败且无备用端口: %w", t.Port, err)
		}
		ln, err = net.Listen("tcp", net.JoinHostPort(t.BindAddress, fmt.Sprint(port)))
		if err != nil {
