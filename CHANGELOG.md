...
- 兼容 VPN Gate 节点 CSV 偶发的未转义双引号，避免节点列表整体解析失败。
- 继续校验必要字段和 OpenVPN 配置 Base64，脏记录不会直接变成可用节点。

## v2.1.4 - 2026-07-28

- “更换节点”改为完整节点选择器，不再点击后立即随机切换。
- 展示全部 VPN Gate 节点的国家、主机名、IP、公布延迟、在线人数和线路速度。
- 支持搜索、国家筛选，以及按速度、延迟、在线人数或国家排序。
- 支持跨国家指定节点切换，保持原 SOCKS5 端口、Mihomo UUID 和分享链接参数不变。
- 当前节点和其他出口正在使用的节点会明确标记并禁止重复选择。
- `/api/nodes` 不再截断为前 200 个节点。
