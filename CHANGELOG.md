...
- 修复入站状态丢失后，Mihomo 中残留的 `fy-*` 用户和规则无法被识别或清理。
- 保存 `native.json` 前自动保留最后一份有效非空备份，空状态不会覆盖恢复点。
- 当状态为空但 Mihomo 中仍存在用户名与 UUID 完全匹配的受管用户时，自动从备份恢复入站。
- 用户主动删除入站后 Mihomo 凭据会同步移除，不会在重启时被误恢复。
