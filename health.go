...
func (m *Manager) tunnelHealthy(t *Tunnel) bool {
	...
	out, err := commandInNetns(t.nsName(), "curl", "-s", "--max-time", strconv.Itoa(int(healthTimeout.Seconds())),
...
func (m *Manager) reconnect(t *Tunnel) {
	...
	if t.ovpn != nil && t.ovpn.Process != nil {
		...
		// Wait briefly before deleting the namespace. cmd.Wait runs in the
		// reaper goroutine and releases the process immediately after SIGKILL.
